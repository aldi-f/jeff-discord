    ...
    async def riven(self, ctx, *, weapon: str = None):
        ...
        try:
            response = requests.get(
                "https://www.warframe.com/repos/weeklyRivensPC.json")
            ...
            if len(name) != 0:
                ...
                res_market = requests.get(
                    f"https://api.warframe.market/v1/auctions/search?type=riven&weapon_url_name={name}&sort_by=price_asc")
                ...
                for x in market:
                    ...
                    if x['owner']['status'] != 'offline':
                        ...
                        for att in x['item']['attributes']:
                            ...
                            symbol = '%'
                            bonus = ' '.join(
                                att['url_name'].split('_')).capitalize()
                            if 'positive' in att:
                                sign = '+'
