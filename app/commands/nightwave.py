...
class nightwave(commands.Cog):
    ...
    @commands.command(
        name="nw",
        description="Show the current Nightwave Rotation",
        aliases=["nightwave", "night"],
    )
    async def nw(self, ctx):
        ...
        embed.set_footer(
            text=f"Latency: {round((time.time() - start) * 1000)}ms{chr(10)}"
        )
        await ctx.send(embed=embed)

    @discord.app_commands.command(
        name="nightwave", description="Show the current Nightwave Rotation"
    )
