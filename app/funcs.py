...
def pricecheck(name: str, rank: int = None):
    if not (
        name.lower().endswith("prime blueprint")
        or name.lower().endswith("collar blueprint")
    ):
    ...
    for trade in trades:
        if "mod_rank" in trade:
            if (
                trade["user"]["status"] == "ingame"
                and trade["order_type"] == "sell"
                and trade["mod_rank"] == rank
                and trade["user"]["platform"] != "switch"
            ):
                if trade["platinum"] < lowest[0]:
                    lowest[0] = trade["platinum"]
...
GITHUB = {
    "ability:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/ability.json",
    "arcane:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/arcane.json",
    "blueprint:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/blueprints.json",
    "companion:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/companions.json",
    "enemy:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/enemies.json",
    ...
    "mod:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/mods.json",
    "skins:1": "https://raw.githubusercontent.com/WFCD/warframe-items/master/data/json/Skins.json",
    "tennogen:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/tennogen.json",
    "void:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/void.json",
    "warframe:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/warframes.json",
    "weapon:1": "https://raw.githubusercontent.com/aldi-f/warframe-wiki-scraper/refs/heads/main/data/weapons.json",
}
...
CHECKSUMS = {
    ...
    "missions:1": "missions:checksum:1",
...
def update_cache(data_key: str, redis_cache: "RedisManager"):
    ...
    while not ready and retries < 100:
        retries += 1
        print(
            f"[refill_wiki_data][{time.ctime()}]:\t[Downloading data for '{data_key}'...]"
        )
        try:
            res = requests.post(url=WIKI_URL_BASE, data=WIKI_MODULE_BODY[data_key])
        except:
            print(
                f"[refill_wiki_data][{time.ctime()}]:\t[Downloading failed '{data_key}'{chr(10)}]"
            )
            continue
        ...
        if return_data:
            ...
            checksum = hashlib.md5(
                bytes("".join(json.dumps(unserialized_data)), encoding="utf-8")
            ).hexdigest()
            cached = False
            ...
            if checksum == old_checksum:
                ...
                if not cached:
                    ...
                    print(
                        f"[refill_wiki_data][{time.ctime()}]:\t[{data_key} data ready on redis!']"
                    )
                else:
                    print(
                        f"[refill_wiki_data][{time.ctime()}]:\t[No new data for {data_key}! Skipping']"
                    )
            else:
                text = json.dumps(unserialized_data)
                redis_cache.cache.set(data_key, text)
            ...
            print(
                f"[refill_wiki_data][{time.ctime()}]:\t[Downloading not succesful for '{data_key}'data retrieved: {data_key}. Retrying...({retries})]"
            )
...
@cache
def find_internal_mission_name(
    internal_name: str, redis_cache: "RedisManager"
) -> str | None:
...
@cache
def find_internal_mission_type(
    internal_name: str, redis_cache: "RedisManager"
) -> str | None:
    if not redis_cache.cache.exists("missions:1"):
        update_cache("missions:1", redis_cache)
...
@cache
def find_internal_warframe_name(
    internal_name: str, redis_cache: "RedisManager"
) -> str | None:
...
@cache
def find_internal_companion_name(
    internal_name: str, redis_cache: "RedisManager"
) -> str | None:
...
@cache
def find_internal_ability_name(
    internal_name: str, redis_cache: "RedisManager"
) -> str | None:
